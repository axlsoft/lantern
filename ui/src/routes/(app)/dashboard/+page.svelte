<script lang="ts">
	...
	import { CircleAlert, GitPullRequest, Clock, TrendingUp, TrendingDown } from 'lucide-svelte';
					...
					{#if data.recent_runs.length === 0}
						<Card class="p-6 text-center">
							<p class="text-sm text-gray-500">
								No runs yet. Instrument your tests to get started.
							</p>
						</Card>
					{:else}
						<Card>
							<ul role="list">
								{#each data.recent_runs as run, i (run.id)}
									<li
										class="flex items-center justify-between gap-3 px-4 py-3 {i > 0
					...
					{:else}
						<Card>
							...
							<div class="border-t border-gray-100 px-4 py-2">
								<a href={`/gap-report/${projectId}`} class="text-sm text-blue-600 hover:underline">
									View all gaps →
								</a>
							</div>
						</Card>
					...
					<Card>
						<ul role="list">
							{#each data.open_prs as pr, i (pr.pr_number)}
								<li
									class="flex items-center justify-between gap-4 px-4 py-3 {i > 0
										? 'border-t border-gray-100'
										: ''}"
								>
									...
									<div class="flex shrink-0 items-center gap-3">
										{#if pr.covered_delta !== 0}
											<span
												class="flex items-center gap-0.5 text-sm font-medium {pr.covered_delta > 0
													? 'text-green-600'
													: 'text-red-600'}"
												aria-label="{pr.covered_delta > 0
													? 'Coverage increased by'
													: 'Coverage decreased by'} {Math.abs(pr.covered_delta)} lines"
											>
												{#if pr.covered_delta > 0}
